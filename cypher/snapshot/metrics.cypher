...
//Distribution of IP Block advertisements by origin
match (a:AS)
...
//Distribution of route advertisements by peer
match (c:Collector)<--(r:Route)-[rel:HAS_AS]->(a:AS)
where rel.order = 1
return c.name as Collector, a.ASN as Peer_ASN, a.name as Peer_Name, count(r) as Count
